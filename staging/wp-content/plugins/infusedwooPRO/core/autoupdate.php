    ...
    {  
        ...
        if (version_compare($this->current_version, $this->remote_version, '<')) {  
            if(empty($this->lic_key))
                ...
                $this->lic_validate   = $this->validateLicense();
...
function iwpro_autoupdate($ifs)  {  
    if( is_admin() ) {        
        $plugin_current_version = INFUSEDWOO_PRO_VER;  

        $plugin_remote_path = INFUSEDWOO_PRO_UPDATER;  
        $plugin_slug = INFUSEDWOO_PRO_BASE;  
        $autoupdate = new ia_auto_update ($plugin_current_version, $plugin_remote_path, $plugin_slug, $ifs->lic_key);

         if(session_id() == '') {
            session_start();
        }
        $_SESSION['iwpro_remote_ver'] = $autoupdate->remote_version; 
        $_SESSION['iwpro_lic_validate'] = $autoupdate->lic_validate;
