	...
	if(isset($email) && !empty($email)) {
		...
		$exists = $wpdb->get_var( $wpdb->prepare(
	  	...
	  	if($exists) {
			$wpdb->update( 
				$ia_savedcarts, 
				array( 'cartcontent' => serialize($cart_contents)), 
				array( 'email' => $email ), 
				array( '%s'), 
				array( '%s' ) 
			);
...
function ia_restore_cart() {
	...
	if(isset($_GET['ia_saved_cart'])) {
		$saved_cart = $wpdb->get_var( $wpdb->prepare(
			    	"SELECT cartcontent FROM `$ia_savedcarts` WHERE `hash` = %s", $_GET['ia_saved_cart']
			  	));
...
add_action('woocommerce_init', 'ia_restore_cart');
