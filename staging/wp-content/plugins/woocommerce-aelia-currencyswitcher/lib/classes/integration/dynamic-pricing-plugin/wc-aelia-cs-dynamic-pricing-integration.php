...
class WC_Aelia_CS_Dynamic_Pricing_Integration {
	 ...
	 * they are applied.
	 *
	 * @param WC_Dynamic_Pricing_Simple_Category module The module to be processed.
	 ...
	 * they are applied.
	 *
	 * @param WC_Dynamic_Pricing_Advanced_Totals module The module to be processed.
	 ...
	 * @since 3.8.4.150824
	 */
	protected function process_simple_membership_rules($module) {
		if(is_array($module->available_rulesets)) {
			foreach($module->available_rulesets as $set_id => $discount) {
				...
				if(!is_array($discount)) {
					Logger::log(sprintf(__('Membership Pricing rule set "%s" - "rules" attribute is not an array. Skipping.'),
															$set_id), true);
					continue;
				}
