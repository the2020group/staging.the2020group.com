...
class WC_PB_Order {
	...
	function woo_bundles_add_order_item_meta( $order_item_id, $cart_item_values, $cart_item_key = '' ) {
		...
		if ( isset( $cart_item_values[ 'bundled_by' ] ) ) {
			...
			$product_key = WC()->cart->find_product_in_cart( $cart_item_values[ 'bundled_by' ] );

			if ( ! empty( $product_key ) ) {

				$product_name = WC()->cart->cart_contents[ $product_key ][ 'data' ]->get_title();

				wc_add_order_item_meta( $order_item_id, __( 'Included with', 'woocommerce-product-bundles' ), $product_name );
			}
