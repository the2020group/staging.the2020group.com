...
class WC_Product_Bundle extends WC_Product {
	...
	public function sync_bundle() {
		...
		foreach ( $this->bundled_items as $bundled_item ) {
			...
			if ( ! $bundled_item->is_purchasable() && ! $bundled_item->is_optional() )
			...
			if ( $this->is_priced_per_product() ) {
				...
				$this->min_bundle_price 		= $this->min_bundle_price + $optional_coeff * $bundled_item->get_quantity() * $bundled_item->min_price;
				$this->min_bundle_regular_price = $this->min_bundle_regular_price + $optional_coeff * $bundled_item->get_quantity() * $bundled_item->min_regular_price;

				$this->max_bundle_price 		= $this->max_bundle_price + $bundled_item->get_quantity() * $bundled_item->max_price;
				$this->max_bundle_regular_price = $this->max_bundle_regular_price + $bundled_item->get_quantity() * $bundled_item->max_regular_price;


				$this->min_bundle_price_excl_tax = $this->min_bundle_price_excl_tax + $optional_coeff * $bundled_item->get_quantity() * $bundled_item->min_price_excl_tax;
				$this->min_bundle_price_incl_tax = $this->min_bundle_price_incl_tax + $optional_coeff * $bundled_item->get_quantity() * $bundled_item->min_price_incl_tax;
	...
	public function get_price() {
		...
		if ( $this->is_priced_per_product() )
			return apply_filters( 'woocommerce_bundle_get_price', (double) max( 0, $this->offset_price ), $this );
		else
