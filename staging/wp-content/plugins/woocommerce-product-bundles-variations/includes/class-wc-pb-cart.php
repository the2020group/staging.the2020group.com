...
class WC_PB_Cart {
        ...
        if ( $product_type == 'bundle' ) {
            ...
            foreach ( $bundled_items as $bundled_item_id => $bundled_item ) {
                ...
                if ( ! $bundled_item->is_purchasable() ) {
                    wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart because &quot;%s&quot; cannot be purchased at the moment.', 'woocommerce-product-bundles' ), get_the_title( $product_id ), $bundled_item->get_raw_title() ), 'error' );
                    return false;
                }
                ...
                if ( $bundled_product_type == 'variable' ) {
                    ...
                    if ( isset( $variation_id ) && is_numeric( $variation_id ) && $variation_id > 1 ) {
                        ...
                        if ( get_post_meta( $variation_id, '_price', true ) === '' ) {
                            ...
                            wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart. The selected variation of &quot;%s&quot; cannot be purchased.', 'woocommerce-product-bundles' ), get_the_title( $product_id ), $bundled_item->product->get_title() ), 'error' );
                            return false;
                        }
                        ...
                        // DS Temo fix to allow bunlded variations
                        //wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart. Please choose &quot;%s&quot; options&hellip;', 'woocommerce-product-bundles' ), get_the_title( $product_id ), $bundled_item->product->get_title() ), 'error' );
                        return true;
                    }
                    ...
                    if ( ! $all_set ) {
                        ...
                        // DS TEMP - fix to allow bundled rpoduct with variations
                        //wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart. Please choose &quot;%s&quot; options&hellip;', 'woocommerce-product-bundles' ), get_the_title( $product_id ), $bundled_item->product->get_title() ), 'error' );
                        return true; 
                    }
    ...
    function woo_bundles_add_cart_item_data( $cart_item_data, $product_id ) {
        ...
        if ( $product_type == 'bundle' ) {
            ...
            foreach ( $bundled_items as $bundled_item_id => $bundled_item ) {
                ...
                $stamp[ $bundled_item_id ][ 'quantity' ]	= $bundled_item->get_quantity();
    ...
    function woo_bundles_add_bundle_to_cart( $bundle_cart_key, $bundle_id, $bundle_quantity, $variation_id, $variation, $cart_item_data ) {
        ...
        if ( isset( $cart_item_data[ 'stamp' ] ) && ! isset( $cart_item_data[ 'bundled_by' ] ) ) {
            ...
            foreach ( $cart_item_data[ 'stamp' ] as $bundled_item_id => $bundled_item_stamp ) {
                ...
                if ( ! in_array( $bundled_item_cart_key, WC()->cart->cart_contents[ $bundle_cart_key ][ 'bundled_items' ] ) )
    ...
    public function bundled_add_to_cart( $bundle_id, $product_id, $quantity = 1, $variation_id = '', $variation = '', $cart_item_data, $add = false ) {
        ...
        if ( $product_data->sold_individually == 'yes' && $quantity > 1 ) {
            if ( ! $add )
                wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart &mdash; only 1 &quot;%s&quot; may be purchased.', 'woocommerce-product-bundles' ), get_the_title( $bundle_id ), $product_data->get_title() ), 'error' );
            return false;
        }
        ...
        if ( ! $product_data->is_in_stock() ) {
            ...
            if ( $product_data->product_type == 'variable' ) {
                if ( ! $add )
                    wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart because the option selected for &quot;%s&quot; is out of stock.', 'woocommerce-product-bundles' ), get_the_title( $bundle_id ), $product_data->get_title() ), 'error' );
            } else {
                if ( ! $add )
                    wc_add_notice( sprintf( __( '&quot;%s&quot; cannot be added to the cart because &quot;%s&quot; is out of stock.', 'woocommerce-product-bundles' ), get_the_title( $bundle_id ), $product_data->get_title() ), 'error' );
            }
            ...
            if ( $product_data->product_type == 'variable' ) {
                if ( ! $add )
                    wc_add_notice( sprintf(__( '&quot;%s&quot; cannot be added to the cart because the option selected for &quot;%s&quot; does not have enough stock (%s remaining).', 'woocommerce-product-bundles' ), get_the_title( $bundle_id ), $product_data->get_title(), $product_data->get_stock_quantity() ), 'error' );
            } else {
                if ( ! $add )
                    wc_add_notice( sprintf(__( '&quot;%s&quot; cannot be added to the cart because there is not enough stock of &quot;%s&quot; (%s remaining).', 'woocommerce-product-bundles' ), get_the_title( $bundle_id ), $product_data->get_title(), $product_data->get_stock_quantity() ), 'error' );
            }
            ...
            if ( isset( $cart_contents[ $bundle_cart_key ] ) ) {
                ...
                if ( $per_product_pricing == false ) {
                    ...
                    if ( ! empty( $discount ) ) {
                        ...
                        $cart_data[ 'data' ]->price = $cart_data[ 'data' ]->get_price();
    ...
    function woo_bundles_get_item_data( $data, $cart_item ) {
        ...
        if ( isset( $cart_item[ 'bundled_by' ] ) && isset( $cart_item[ 'stamp' ] ) ) {
            ...
            $product_key = WC()->cart->find_product_in_cart( $cart_item[ 'bundled_by' ] );

            if ( ! empty( $product_key ) ) {

                $product_name = get_post( WC()->cart->cart_contents[ $product_key ][ 'product_id' ] )->post_title;
                $data[] = array(
    ...
    function woo_bundles_cart_widget_filter( $show, $cart_item, $cart_item_key ) {
        ...
        if ( isset( $cart_item[ 'bundled_by' ] ) ) {
            ...
            $bundle_key = WC()->cart->find_product_in_cart( $cart_item[ 'bundled_by' ] );

            if ( ! empty( $bundle_key ) ) {

                $bundle_product = WC()->cart->cart_contents[ $bundle_key ][ 'data' ];
    ...
    function woo_bundles_before_cart() {
		...
		echo 'Cart Contents Total: ' . WC()->cart->cart_contents_total . '<br/>';
		echo 'Cart Tax Total: ' . WC()->cart->tax_total . '<br/>';
		echo 'Cart Total: ' . WC()->cart->get_cart_total() . '<br/>';
