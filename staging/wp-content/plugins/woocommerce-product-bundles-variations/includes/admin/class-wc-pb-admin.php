...
class WC_PB_Admin {
	...
	function bundled_product_admin_config_html( $loop, $product_id, $item_data, $post_id ) {
		...
		if ( $bundled_product->product_type == 'variable' ) {
			...
			?><div class="filtering"><?php

				?><p class="tip">
					<a class="tips" data-tip="<?php _e( 'Check to enable only a subset of the available variations.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
				</p>

				<p class="form-field filter_variations">
					<label for="filter_variations"><?php echo __( 'Filter Variations', 'woocommerce-product-bundles' ) ?></label>
			...
			<div class="bundle_variation_filters indented">
				...
				<select multiple="multiple" name="bundle_data[<?php echo $loop; ?>][allowed_variations][]" style="width: 75%;" data-placeholder="<?php _e( 'Choose variations&hellip;', 'woocommerce-product-bundles' ); ?>" class="<?php echo WC_PB_Core_Compatibility::is_wc_version_gte_2_3() ? 'wc-enhanced-select' : 'chosen_select'; ?>" > <?php
			...
			<div class="defaults">
				<p class="tip">
					<a class="tips" data-tip="<?php _e( 'In effect for this bundle only. The available options are in sync with the filtering settings above. Always save any changes made above before configuring this section.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
				</p>

				<p class="form-field override_defaults">
		...
		$item_quantity = ! empty( $item_data[ 'bundle_quantity' ] ) ? $item_data[ 'bundle_quantity' ] : 1;
		...
		if ( ! $is_sub ) : ?>
			...
			<div class="optional">
				<p class="tip">
					<a class="tips" data-tip="<?php _e( 'Check this option to mark the bundled product as optional.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
				</p>

				<p class="form-field optional">
		...
		<?php do_action( 'woocommerce_bundled_product_admin_config_html_after_qty', $loop, $product_id, $item_data, $post_id ); ?>

		<div class="discount">
			<p class="tip">
				<a class="tips" data-tip="<?php _e( 'Discount applied to the regular price of this bundled product when Per-Item Pricing is active. Note: If a Discount is applied to a bundled product which has a sale price defined, the sale price will be overridden.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
			</p>
		...
		<div class="item_visibility">
			...
			<p class="form-field">
				...
				<select name="bundle_data[<?php echo $loop; ?>][visibility]">
					<?php
		...
		<div class="images">
			<p class="tip">
				<a class="tips" data-tip="<?php _e( 'Check this option to hide the thumbnail image of this bundled product.', 'woocommerce-product-bundles' ); ?>" >[?]</a>
			</p>

			<p class="form-field hide_thumbnail">
		...
		<div class="override_title">
			<p class="tip">
				<a class="tips" data-tip="<?php _e( 'Check this option to override the default product title.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
			</p>

			<p class="form-field override_title">
		...
		<div class="custom_title">
			<p class="form-field product_title">
				<label><?php echo __( 'Product Title:', 'woocommerce-composite-products' ); ?>:</label><?php
		...
		<div class="override_description">
			<p class="tip">
				<a class="tips" data-tip="<?php _e( 'Check this option to override the default short product description.', 'woocommerce-product-bundles' ); ?>" href="#" >[?]</a>
			</p>

			<p class="form-field override_description">
		...
		<div class="custom_description">
			<p class="form-field product_description">
				<label><?php echo __( 'Product Short Description:', 'woocommerce-composite-products' ); ?>:</label><?php
				...
				if ( ( $id && $id > 0 ) && ( $product_type == 'simple' || $product_type == 'variable' || $is_sub ) && ( $post_id != $id ) ) {
					...
					if ( $is_sub ) {
						...
						if ( $bundled_subs > 0 ) {
							...
							$this->save_errors[] = $this->add_admin_error( sprintf( __( '\'%s\' (#%s) was not saved. Only one simple Subscription per Bundle is supported.', 'woocommerce-product-bundles' ), get_the_title( $id ), $id ) );
							continue;
					...
					if ( ! isset( $times[ $id ] ) ) {
						...
						if ( get_post_meta( $id, '_sold_individually', true ) == 'yes' ) {
							...
							$this->save_errors[] = $this->add_admin_error( sprintf( __( '\'%s\' (#%s) is sold individually and cannot be bundled more than once.', 'woocommerce-product-bundles' ), get_the_title( $id ), $id ) );
							continue;
					...
					if ( isset( $data[ 'bundle_quantity' ] ) ) {
						...
						if ( is_numeric( $data[ 'bundle_quantity' ] ) ) {
							...
							$quantity = (int) $data[ 'bundle_quantity' ];
							...
							if ( $quantity > 0 && $data[ 'bundle_quantity' ] - $quantity == 0 ) {
								...
								if ( $quantity != 1 && ( get_post_meta( $id, '_sold_individually', true ) == 'yes' || ( get_post_meta( $id, '_downloadable', true ) == 'yes' && get_post_meta( $id, '_virtual', true ) == 'yes' && get_option( 'woocommerce_limit_downloadable_product_qty' ) == 'yes' ) ) ) {
									...
									$this->save_errors[] = $this->add_admin_error( sprintf( __( '\'%s\' (#%s) is sold individually and cannot be bundled with a quantity higher than 1.', 'woocommerce-product-bundles' ), get_the_title( $id ), $id ) );
									$bundle_data[ $val ][ 'bundle_quantity' ] = 1;

								} else {
									$bundle_data[ $val ][ 'bundle_quantity' ] = (int) $data[ 'bundle_quantity' ];
								}

							} else {

								$this->save_errors[] = $this->add_admin_error( sprintf( __( 'The quantity you entered for \'%s%s\' (#%s) was not valid and has been reset. Please enter a positive integer value.', 'woocommerce-product-bundles' ), get_the_title( $id ), ( $id != $val ? ' #' . $times[ $id ] : '' ), $id ) );
							}
						}
					...
					if ( isset( $data[ 'bundle_discount' ] ) ) {
						...
						if ( is_numeric( $data[ 'bundle_discount' ] ) ) {
							...
							if ( $discount < 0 || $discount > 100 ) {
								...
								$this->save_errors[] = $this->add_admin_error( sprintf( __( 'The discount value you entered for \'%s%s\' (#%s) was not valid and has been reset. Please enter a positive number between 0-100.', 'woocommerce-product-bundles' ), get_the_title( $id ), ( $id != $val ? ' #' . $times[$id] : '' ), $id ) );
					...
					if ( $product_type == 'variable' ) {
						...
						if ( isset( $data[ 'filter_variations' ] ) ) {
							...
							if ( isset( $data[ 'allowed_variations' ] ) && count( $data[ 'allowed_variations' ] ) > 0 ) {
								...
								$this->save_errors[] = $this->add_admin_error( __('Please select at least one variation for each bundled product you want to filter.', 'woocommerce-product-bundles') );
							}
						} else {
							$bundle_data[ $val ][ 'filter_variations' ] = 'no';
						}
									...
									if ( isset( $data[ 'default_attributes' ] ) ) {
										...
										foreach ( $data[ 'default_attributes' ] as $default_name => $default_value ) {
											...
											if ( ! $default_value ) {
												...
												$this->save_errors[] = $this->add_admin_error( sprintf( __( '\'%s%s\' (#%s) cannot be hidden unless all default options of the product are defined.', 'woocommerce-product-bundles' ), get_the_title( $id ), ( $id != $val ? ' #' . $times[$id] : '' ), $id ) );
												break;
											}
										}
