jQuery( function($) {
	...
	function init_wc_bundle_metaboxes() {
		...
		.on( 'change', '.override_title > p input', function() {
			if ( $(this).is( ':checked' ) )
		...
		.on( 'change', '.override_description > p input', function() {
			if ( $(this).is( ':checked' ) )
		...
		$( '.wc-bundled-items .override_title > p input' ).change();
		$( '.wc-bundled-items .override_description > p input' ).change();
	...
	$( '#bundled_product_data' ).on( 'click', 'button.add_bundled_product', function() {
		...
		$.post( woocommerce_admin_meta_boxes.ajax_url, data, function ( response ) {
			...
			if ( response.markup !== '' ) {
				...
				added.find( '.override_title > p input' ).change();
				added.find( '.override_description > p input' ).change();
