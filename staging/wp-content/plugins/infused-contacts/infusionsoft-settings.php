...
class Infusionsoft_Settings {
	...
	public function init() {
		...
		add_settings_field( 'batch_size', 'Batch Size', array( $this, 'batch_size' ), 'infusionsoftwp', 'infusionsoft_api' );
		add_settings_field( 'enable_sync', 'Enable Syncing', array( $this, 'enable_sync' ), 'infusionsoftwp', 'infusionsoft_api' );
	...
	public function menu() {
		add_options_page( 'Infusionsoft Contacts', 'Infusionsoft Contacts', 'manage_options', 'infusionsoft', array( $this, 'settings_page' ) );
	}
	...
	public function batch_size() {
		...
		echo '<input type="text" size="10" name="infusionsoft_settings[batch_size]" value="' . esc_attr( $batch_size ) . '" />';
	}


	public function enable_sync() {
		$settings = (array) get_option( 'infusionsoft_settings' );
		if ( isset( $settings['enable_sync'] ) ) {
			$enable_sync = $settings['enable_sync'];
		} else {
			$enable_sync = null;
		}
		echo '<input type="checkbox" size="40" name="infusionsoft_settings[enable_sync]" value="' . esc_attr( $enable_sync ) . '" />';
	}
	...
	public function settings_page() {
		...
		<div class="wrap">
			...
			<h2>Infusionsoft Proactive Contact checker and Updater</h2>
