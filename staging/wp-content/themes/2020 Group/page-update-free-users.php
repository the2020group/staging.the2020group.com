...
/*
 * Template Name: Update Comp Users
 */
...
$file = fopen(__DIR__.'/library/imports/complimentary-members.csv','r');
    ...
    while(($cont = fgetcsv($file,9000,',')) !== FALSE) {
        ...
        if ($line >= $start && $line < $end && $cont[0] !='' && isset($cont[0])&& $cont[6]!='') {
            error_log($line);
            $user = array();
            ...
            if (1==1 || trim($user['is_company_id'])=='8312') :
                ...
                elseif (in_array('020103 Mem - Individual Membership (Industry)', $user['tags']['02 Behavioural Tags'])) {
                    ...
                    error_log('oops not sure what membership : '.print_r($user['tags'],1));
                    $continue = false;
                ...
                if ($continue) {
                    ...
                    if ( $user_id && email_exists($user['email']) == true ) {

                        error_log($line.':'.$user['is_contact_id'].' - '.$user['is_company_id']);


                        if ($user_id == 0) {
                            error_log( 'User not found:'.print_r($user['email'],1));
                        }
                        else {
                            ...
                            if ($type == 'Partner' || $type == 'Employee') {
                                ...
                                if ($p_id > 0) {
                                    ...
                                    echo '<br>sm order created: '.$order->id;
                                    ...
                                    wp_update_post($update_these);
                                    update_post_meta($order->id, '_completed_date', $start_date);
                                    update_post_meta($order->id, '_paid_date', $start_date);
                                    // End change dates
                                ...
                                error_log('sm order NOT created: '.$line.'-'.$user['email']);
                                echo '<br> '.$line.' sm order NOT created: '.$line.'-'.$user['email'];
                            }
