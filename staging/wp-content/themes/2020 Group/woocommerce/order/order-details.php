		...
		if ( sizeof( $order->get_items() ) > 0 ) {
			...
			foreach( $order->get_items() as $item_id => $item ) {
				...
				if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
					...
					<?php
					// if this product is a subscription  - get the sign up fee and display
					...
					if ( $subscription_order && $signupfee >0 ) {
						echo '<tr><td class="product-name">Subscription Sign-Up Fee</td><td class="product-total">'.get_woocommerce_currency_symbol().current_ex_rate($signupfee).'</td></tr>';
					}
